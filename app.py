...
estoque_acessorios = {
   ...
   "BRAÇO MAX AR 25cm": 13.00,
   "BRAÇO MAX AR 35cm": 17.00,
   "BRAÇO MAX AR 55cm": 23.00,
   ...
   "BUCHA 8": 0.30,
   "CANOPLA GRADIL": 3.00,
   "CAVALETE BOX VELHO": 1.00,
   "CAVALETE BOX NOVO": 1.00,
   "CHAPA ACRÍLICA CRISTAL": 130.00,
   "CHAPA ACRÍLICA FUME": 110.00,
   "CONTROLE MOTOR": 40.00,
   "CREMALHEIRA 1,50MT": 55.00,
   "DOBRADIÇA BOX": 3.00,
   "DOBRADIÇA BÚZIO": 9.00,
   "DOBRADIÇA PORTA": 5.00,
   "FECHADURA CORRER BÚZIO": 75.00,
   "FECHADURA DE PORTA CORRER": 50.00,
   "FECHADURA DE PORTA GIRO": 75.00,
   "FECHADURA PORTA 401": 78.00,
   "FECHADURA BÚZIO GIRO 1/2": 92.00,
   "FECHADURA BÚZIO COMPLETA": 97.00,
   "FECHADURA ROLETE BÚZIO": 85.00,
   "FECHADURA ROLETE PORTA": 70.00,
   "FECHADURA GRADE 701": 60.00,
   "FECHO AVIÃO CROMADO": 6.00,
   "FECHO AVIÃO NYLON": 5.00,
   "FECHO CONCHA C/ GATILHO": 15.00,
   "FECHO MAXIM AR": 11.00,
   "FERROLHO 2'": 8.00,
   "FERROLHO 3'": 9.00,
   "FERROLHO 4'": 10.00,
   "FERROLHO 40CM": 20.00,
   "FERROLHO 20CM": 15.00,
   "FITA ADESIVA 11X4": 10.00,
   "FITA ADESIVA 11X6": 12.00,
   "FIXA ESPELHO": 20.00,
   "GUIA BOX VELHO": 0.25,
   "GUIA BÚZIO": 5.00,
   "GUIA BÚZIO NYLON": 5.00,
   "GUIA DE BOX NOVO": 1.00,
   "GUIA JANELA MB": 0.50,
   "GUIA JANELA MP": 0.50,
   "GUIA JANELA SUPREMA": 2.00,
   "H PANORAMICO": 4.00,
   "H POLICARBONATO": 120.00,
   "JOGO L E CUNHA": 45.00,
   "L. PARA CONTRA MARCO": 2.00,
   "L. PARA BOX NYLON": 0.60,
   "L. PARA BOX ALUMÍNIO": 0.60,
   "L. PERFIL DE TELA": 2.00,
   "MOTOR": 450.00,
   "ORELHA DE RATO L 25": 7.00,
   "ORELHA  DE RATO MP": 7.00,
   "PARAFUSO PONTA LISA": 0.30,
   "POLICARBONATO": 440.00,
   "PONTALETE 30CM": 25.00,
   "PONTALETE 60CM": 40.00,
   "PUXADOR BOX SIMPLES": 1.50,
   "PUXADOR BOX DUPLO": 2.50,
   "POLIETILENO DE 10'": 17.00,
   "ROLDANA BOX": 1.00,
   "ROLDANA BÚZIO AÇO": 22.00,
   "ROLDANA BUZIO NYLON": 18.00,
   "ROLDANA EXCENTRICA": 6.50,
   "ROLDANA JANELA SUPREMA": 6.00,
   "ROLDANA PORTA SUPREMA": 12.00,
   "ROLDANA MB": 1.00,
   "ROLDANA MP": 1.50,
   "ROLDANA MP C/ ROLAMENTO": 5.50,
   "ROLDANA PORTA MP NYLON": 7.50,
   "ROLDANA PORTA MP ALUMÍNIO": 7.50,
   "ROLDANA SIMPLES": 3.00,
   "ROLDANA STANLEY (PAR)": 25.00,
   "SILICONE INCOLOR": 15.00,
   "SILICONE BC, PT E BZ": 18.00,
   "SILICONE BRONZE WURTH": 25.00,
   "SPRAY": 25.00,
   "SUPORTE PARA CORRIMÃO": 9.00,
   "TAMPA CORRIMÃO BOLEADO": 3.00,
   "TAMPA CORRIMÃO REDONDO": 5.00,
   "TRANQUETA MAX AR": 11.00,
   "Z GRADIL": 4.00,
...
def criar_tabelas():
    ...
    # Atualiza o banco antigo para aceitar a nova coluna de Categoria sem apagar dados
    try:
...
if menu == "🛒 Orçamentos e Vendas":
    ...
    c.execute('SELECT categoria, perfil, cor, preco_metro FROM produtos')
    ...
    if not produtos_db:
        ...
        with col2:
            ...
            st.info(f"Valor: **R$ {valor_calculado:.2f}**")
    ...
    if st.session_state["carrinho"]:
        ...
        with col_fim2:
            if st.button("🖨️ Imprimir Pedido", use_container_width=True):
            ...
            if tipo_registro_selecionado == "✅ Venda Definitiva":
                ...
                for item in st.session_state['carrinho']:
                    ...
                    c.execute('INSERT INTO vendas (data_venda, cliente, produto, quantidade, valor_total, vendedor) VALUES (?, ?, ?, ?, ?, ?)', 
                              (data_atual, cliente_selecionado, nome_banco, item['Qtd'], item['Valor (R$)'], st.session_state['usuario_atual']))
                conn.commit()
                
            valor_pedido = sum(item["Valor (R$)"] for item in st.session_state["carrinho"])
            html_recibo = gerar_html_3_vias(st.session_state['carrinho'], valor_pedido, cliente_selecionado, tel_selecionado, end_selecionado, 
                                            "VENDA" if tipo_registro_selecionado == "✅ Venda Definitiva" else "ORCAMENTO")
    ...
    if df_vendas.empty:
        ...
        st.altair_chart(alt.Chart(vendas_agrupadas).mark_bar().encode(
            x='produto', y='valor_total', color='produto'
